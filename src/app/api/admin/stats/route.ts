...
export async function GET() {
  ...
  // Everything below is for the course the admin is viewing.
  const lang = await getLang();
  const accounts = await listAccounts();
  const students = accounts.filter((a) => !a.isAdmin && studiesLanguage(a, lang));
  const byDateAsc = [...students].sort((a, b) => (a.createdAt < b.createdAt ? -1 : 1));

  const perLevel: Record<string, number> = {};
  for (const l of CEFR_LEVELS) perLevel[l.level] = 0;
  for (const s of students) {
    const lvl = levelFor(s, lang);
  ...
  const states = await listAllStates(lang);
  ...
  for (const s of students) {
    ...
    const n = scheduleFor(s, lang)?.days?.length ?? 0;
    if (!n) continue;
    const lvl = levelFor(s, lang);
    const price = pricing[lvl] ?? 0;
    weeklyClasses += n;
    weeklyIncome += n * price;
    incomeByLevel[lvl] += n * price * WEEKS_PER_MONTH;
  }
  ...
  return NextResponse.json({
    ...
    adminCount: accounts.filter((a) => a.isAdmin).length,
